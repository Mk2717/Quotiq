...
import { ArrowDown, ArrowLeft, ArrowUp, Check, CheckCircle2, Copy, Eye, FileText, LayoutDashboard, Plus, Printer, ReceiptText, Save, Search, Send, Trash2, Users } from 'lucide-react';
...
function EstimateDetail({id}:{id:string}){
 ...
 const estimate=estimates.find(e=>e.id===id),customers=getStored<Customer[]>('q-customers',[]),invoices=getStored<Invoice[]>('q-invoices',[]);
 ...
 <div className="ev2DetailGrid"><aside className="ev2DetailPanel"><h2>Next action</h2>{estimate.status==='Draft'&&<><p>Finalize this estimate and mark it ready for the customer.</p><button className="ev2Primary" onClick={()=>updateStatus('Pending')}><Send/>Mark as sent</button></>}{estimate.status==='Pending'&&<><p>Record the customer’s decision when approval is received.</p><button className="ev2Primary" onClick={()=>updateStatus('Accepted')}><CheckCircle2/>Record acceptance</button></>}{estimate.status==='Accepted'&&!linked&&<><p>The estimate is approved and ready for billing.</p><button className="ev2Primary" onClick={convert}><ReceiptText/>Create invoice</button></>}{linked&&<><p>This estimate is connected to invoice <b>{linked.id}</b>.</p><button onClick={()=>navigate(`/invoices/${encodeURIComponent(linked.id)}`)}><Eye/>View invoice</button></>}<hr/><h3>Customer</h3><b>{estimate.customer}</b><span>{customer?.phone}</span><span>{customer?.email}</span><span>{customer?.siteAddress||customer?.address}</span></aside>
 <PrintableQuote business={business} quoteNo={estimate.id} customer={estimate.customer} phone={customer?.phone||''} email={customer?.email||''} location={customer?.siteAddress||customer?.address||''} project={estimate.project} reference={estimate.reference||''} estimateTrade={estimate.trade||'Professional contractor'} date={estimate.date} validDays={estimate.validDays||14} items={estimate.items} notes={estimate.notes||'Supply, installation, testing and handover of the listed items and services.'} terms={estimate.terms||business.terms||'Quotation is valid for the stated period.'} paymentDetails={estimate.paymentDetails||business.mobileMoney||business.bank||'Payment details will be provided upon acceptance.'} totals={{subtotal,discount,tax,total}}/>
 </div></main>;
}
