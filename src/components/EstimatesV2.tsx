...
function PrintableQuote({business,quoteNo,customer,phone,email,location,project,reference,date,validDays,items,notes,terms,paymentDetails,totals}:any){return <article className="ev2Quote printArea"><div className="ev2Watermark">{business.logo?<img src={business.logo} alt=""/>:<span>Q</span>}</div><header className="ev2QuoteHeader"><div className="ev2Company">{business.logo?<img src={business.logo} alt="Company logo"/>:<div className="ev2LogoFallback">Q</div>}<div><h2>{business.name||'Your Company'}</h2><p>{business.address||'Business address'}</p><p>{[business.phone,business.email].filter(Boolean).join(' · ')}</p>{business.taxId&&<p>Tax ID: {business.taxId}</p>}</div></div><div className="ev2QuoteTitle"><span>ESTIMATE / QUOTATION</span><h1>QUOTATION</h1><strong>{quoteNo}</strong><p>Issued: {prettyDate(date)}</p><p>Valid for: {validDays} days</p></div></header><div className="ev2Accent"/><section className="ev2QuoteMeta"><div><span>PREPARED FOR</span><h3>{customer}</h3>{phone&&<p>{phone}</p>}{email&&<p>{email}</p>}{location&&<p>{location}</p>}</div><div><span>PROJECT</span><h3>{project}</h3>{reference&&<p>Reference: {reference}</p>}</div></section><div className="ev2TableWrap"><table><thead><tr><th>#</th><th>Description</th><th>Qty</th><th>Unit</th><th>Unit price</th><th>Total cost</th></tr></thead><tbody>{items.map((item:DraftLine,index:number)=><tr key={item.id}><td>{String(index+1).padStart(2,'0')}</td><td><b>{item.description}</b>{item.details&&<small>{item.details}</small>}{item.category&&<em>{item.category}</em>}</td><td>{item.qty}</td><td>{item.unit}</td><td>{money(item.rate,business.currency)}</td><td><strong>{money(lineTotal(item),business.currency)}</strong></td></tr>)}</tbody></table></div><section className="ev2Totals"><div><span>Subtotal</span><b>{money(totals.subtotal,business.currency)}</b></div>{totals.discount>0&&<div><span>Discount</span><b>- {money(totals.discount,business.currency)}</b></div>}{totals.tax>0&&<div><span>Tax</span><b>{money(totals.tax,business.currency)}</b></div>}<div className="ev2Grand"><span>TOTAL COST</span><strong>{money(totals.total,business.currency)}</strong></div></section><section className="ev2QuoteDetails"><div><h4>SCOPE OF WORK / NOTES</h4><p>{notes}</p><h4>TERMS & CONDITIONS</h4><p>{terms}</p></div><aside><h4>PAYMENT DETAILS</h4><p>{paymentDetails}</p><div className="ev2Signature"><div>{business.signature&&<img src={business.signature} alt="Signature"/>}</div><span>Authorised signature</span></div></aside></section><footer><div><b>Thank you for the opportunity to quote.</b><span>This document was prepared for {customer}.</span></div><small>Generated with Quotiq</small></footer></article>}
